...
var Methods = {
  ...
  getRadioReserveRecord: function getRadioReserveRecord(isFull) {
    ...
    if (radioOpts.reserve && radioReserveRow) {
      if (isFull ? !fullDataRowIdData[getRowid(this, radioReserveRow)] : !afterFullData.some(function (row) {
        return getRowid(_this30, row) === getRowid(_this30, radioReserveRow);
      })) {
  ...
  getCheckboxReserveRecords: function getCheckboxReserveRecords(isFull) {
    ...
    if (checkboxOpts.reserve) {
      _xeUtils.default.each(checkboxReserveRowMap, function (row, rowid) {
        if (row && (isFull ? !fullDataRowIdData[rowid] : !afterFullData.some(function (item) {
          return getRowid(_this31, item) === rowid;
        }))) {
        ...
        if (editOpts.mode === 'row') {
          this.checkValidate('blur').catch(function (e) {
            ...
            _this34.handleActived(params, evnt).then(function () {
              return _this34.checkValidate('change');
            }).catch(function (e) {
              return e;
            });
          });
    ...
    if (defaultSort) {
      ...
      if (defaultSort.length) {
        (sortConfig.multiple ? defaultSort : defaultSort.slice(0, 1)).forEach(function (item) {
          ...
          if (field && order) {
            var column = _this35.getColumnByField(field);
    ...
    if (sortConfs.length) {
      ...
      (multiple ? sortConfs : [sortConfs[0]]).forEach(function (confs) {
        ...
        if (_xeUtils.default.isString(field)) {
          column = _this36.getColumnByField(field);
        }
        ...
        if (column && (column.sortable || column.remoteSort)) {
          ...
          if (orders.indexOf(order) === -1) {
            order = getNextSortOrder(_this36, column);
          }
  ...
  reloadExpandContent: function reloadExpandContent(row) {
    ...
    if (lazy && expandLazyLoadeds.indexOf(row) === -1) {
      this.clearRowExpandLoaded(row).then(function () {
        return _this37.handleAsyncRowExpand(row);
      });
    }
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    var rest = this.fullAllDataRowMap.get(row);
    return new Promise(function (resolve) {
      _this38.expandLazyLoadeds.push(row);

      _this38.expandOpts.loadMethod({
        $table: _this38,
        row: row,
        ...
        _xeUtils.default.remove(_this38.expandLazyLoadeds, function (item) {
          return item === row;
        });

        _this38.rowExpandeds.push(row);
    ...
    if (rows) {
      ...
      var validRows = toggleMethod ? rows.filter(function (row) {
        return toggleMethod({
          ...
          $rowIndex: _this39.getVMRowIndex(row)
        });
      }) : rows;
      ...
      if (expanded) {
        validRows.forEach(function (row) {
          if (rowExpandeds.indexOf(row) === -1) {
            ...
            if (isLoad) {
              lazyRests.push(_this39.handleAsyncRowExpand(row));
            } else {
      ...
      if (reserve) {
        validRows.forEach(function (row) {
          return _this39.handleRowExpandReserve(row, expanded);
        });
      }
    }
  ...
  clearRowExpand: function clearRowExpand() {
    ...
    if (reserve) {
      tableFullData.forEach(function (row) {
        return _this40.handleRowExpandReserve(row, false);
      });
    }
  ...
  handleAsyncTreeExpandChilds: function handleAsyncTreeExpandChilds(row) {
    ...
    return new Promise(function (resolve) {
      ...
      loadMethod({
        ...
        if (childRecords) {
          _this42.loadChildren(row, childRecords).then(function (childRows) {
            if (childRows.length && treeExpandeds.indexOf(row) === -1) {
              treeExpandeds.push(row);
            } // 如果当前节点已选中，则展开后子节点也被选中


            if (!checkStrictly && _this42.isCheckedByCheckboxRow(row)) {
              _this42.setCheckboxRow(childRows, true);
            }
          });
        }
  ...
  setTreeExpand: function setTreeExpand(rows, expanded) {
    ...
    if (rows) {
      ...
      if (rows.length) {
        ...
        if (expanded) {
          validRows.forEach(function (row) {
            if (treeExpandeds.indexOf(row) === -1) {
              ...
              if (isLoad) {
                result.push(_this43.handleAsyncTreeExpandChilds(row));
              } else {
        ...
        if (reserve) {
          validRows.forEach(function (row) {
            return _this43.handleTreeExpandReserve(row, expanded);
          });
        }
  ...
  clearTreeExpand: function clearTreeExpand() {
    ...
    if (reserve) {
      _xeUtils.default.eachTree(tableFullData, function (row) {
        return _this44.handleTreeExpandReserve(row, false);
      }, treeOpts);
    }
  ...
  scrollToTreeRow: function scrollToTreeRow(row) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this47.isTreeExpandByRow(row)) {
            rests.push(_this47.setTreeExpand(row, true));
          }
        });
      }
    }
    ...
    return Promise.all(rests).then(function () {
      return _tools.DomTools.rowToVisible(_this47, row);
    });
  },
